...
type TowerAOIManager struct {
	minX, maxX, minY, maxY common.Coord
	towerRange             common.Coord
	towers                 [][]tower
	xTowerNum, yTowerNum   int
}

func (aoiman *TowerAOIManager) Enter(aoi *AOI, x, y common.Coord) {
	aoi.x, aoi.z = x, y
	obj := &aoiobj{aoi: aoi}
	aoi.implData = obj
...
func (aoiman *TowerAOIManager) Leave(aoi *AOI) {
	...
	aoiman.visitWatchedTowers(aoi.x, aoi.z, aoi.dist, func(tower *tower) {
		tower.removeWatcher(obj)
	})
}

func (aoiman *TowerAOIManager) Moved(aoi *AOI, x, y common.Coord) {
	oldx, oldy := aoi.x, aoi.z
	aoi.x, aoi.z = x, y
	obj := aoi.implData.(*aoiobj)
	t0 := obj.tower
...
func (aoiman *TowerAOIManager) transXY(x, y common.Coord) (int, int) {
...
func (aoiman *TowerAOIManager) getTowerXY(x, y common.Coord) *tower {
	xi, yi := aoiman.transXY(x, y)
	return &aoiman.towers[xi][yi]
}

func (aoiman *TowerAOIManager) getWatchedTowers(x, y common.Coord, aoiDistance common.Coord) (int, int, int, int) {
	...
	//ximid, yimid := aoiman.transXY(x, z)
...
func (aoiman *TowerAOIManager) visitWatchedTowers(x, y common.Coord, aoiDistance common.Coord, f func(*tower)) {
...
func NewTowerAOIManager(minX, maxX, minY, maxY common.Coord, towerRange common.Coord) AOIManager {
