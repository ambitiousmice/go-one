...
func randCoord(min, max common.Coord) common.Coord {
	return min + common.Coord(rand.Intn(int(max)-int(min)))
}
...
func testAOI(t *testing.T, manname string, aoiman AOIManager, numAOI int) {
	...
	for i := 0; i < 10; i++ {
		...
		for _, obj := range objs {
			aoiman.Moved(&obj.aoi, obj.aoi.x+randCoord(-10, 10), obj.aoi.z+randCoord(-10, 10))
			aoiman.Leave(&obj.aoi)
			aoiman.Enter(&obj.aoi, obj.aoi.x+randCoord(-10, 10), obj.aoi.z+randCoord(-10, 10))
		}
		dt := time.Now().Sub(t0)
